...
const OrderHistory = ({ onOrderClick }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 lg:p-6 pb-24 lg:pb-6 space-y-4 lg:space-y-6 animate-in fade-in duration-700">
      
      {/* ── Header ─────────────────────────────────────────────────── */}
      <header className="flex flex-col gap-4">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-lg lg:text-[1.7rem] font-black text-slate-900 tracking-tight uppercase">Order History</h1>
            <p className="text-[8px] lg:text-[10px] font-semibold text-slate-400 uppercase tracking-[0.1em] mt-1">Registry Archive v3.0</p>
          </div>
          <div className="text-xs font-bold text-slate-400 bg-white px-3 py-1.5 rounded-lg border border-slate-100 shadow-sm hidden sm:block">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <div className="flex gap-2 shrink-0">
            {['All', 'Dine-in', 'Parcel'].map(type => (
              <button
                ...
                className={cn(
                  "px-3 lg:px-5 py-2 rounded-lg lg:rounded-xl text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap",
                  filterType === type
          ...
          <div className="relative group w-full sm:w-auto sm:ml-auto">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-orange-500 transition-colors" size={15} />
            <input
              type="text"
              placeholder="Search by order #, customer, phone..."
              className="bg-white border border-slate-100 rounded-lg lg:rounded-xl py-2.5 pl-10 pr-4 focus:outline-none focus:ring-4 focus:ring-orange-500/5 focus:border-orange-500 transition-all text-xs lg:text-sm font-bold w-full sm:w-72 shadow-sm"
              value={searchTerm}
      ...
      <div className="space-y-5 animate-in zoom-in-95 duration-500">
        ...
        {['Dine-in', 'Parcel'].map(sectionType => {
          ...
          return (
            <div key={sectionType}>
              {/* Section Header */}
              <div className="bg-white rounded-2xl lg:rounded-t-[2rem] lg:rounded-b-none border border-slate-100 lg:border-b-0 px-4 lg:px-8 py-4 lg:py-5 flex items-center justify-between mb-3 lg:mb-0">
                <div className="flex items-center gap-2 lg:gap-3">
                  {sectionType === 'Dine-in' ? <Utensils className="text-orange-500" size={16} /> : <Package className="text-emerald-500" size={16} />}
                  <h2 className="text-xs lg:text-sm font-black text-slate-800 uppercase tracking-widest">{sectionType} Orders</h2>
                </div>
                <div className="text-[10px] lg:text-xs font-bold text-slate-400 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100">
              ...
              {loadingHistory ? (
                <div className="bg-white rounded-b-2xl lg:rounded-b-[2rem] border border-t-0 border-slate-100 py-16 text-center mb-5">
                ...
                <>
                  ...
                  <div className="lg:hidden space-y-3 mb-5">
                    {sectionOrders.map((order) => {
                      ...
                      const statusColor = isCancelled ? 'bg-rose-50 text-rose-600 border-rose-100' :
                        isPaid ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100';
                      const cardBorder = isCancelled ? 'border-rose-100' : isPaid ? 'border-emerald-100' : 'border-slate-100';
                      return (
                        <div
                          key={order._id}
                          className={cn("bg-white rounded-2xl border shadow-sm p-4 active:scale-[0.98] transition-all cursor-pointer", cardBorder)}
                          onClick={() => onOrderClick && onOrderClick(order._id)}
                        >
                          {/* Top row: Order # + Status + Amount */}
                          <div className="flex items-center justify-between mb-2.5">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-black text-slate-900">#{order.orderNumber ? formatOrderNumber(order.orderNumber) : '----'}</span>
                              <span className={cn('px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest', statusColor)}>
                          ...
                          {/* Bottom row: Customer + Phone + Time */}
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <span className={cn("text-xs font-bold", order.customerName ? "text-slate-600" : "text-slate-300")}>
                                {order.customerName || 'Walk-in'}
                              </span>
                              {order.customerPhone && (
                                <span className="text-[11px] font-bold text-slate-400">{order.customerPhone}</span>
                              )}
                            </div>
                            <div className="flex items-center gap-1 text-slate-400">
                              <Clock size={10} />
                              <span className="text-[10px] font-bold">{fmtDate(order.createdAt)} {fmtTime(order.createdAt)}</span>
                            </div>
                          </div>
                        </div>
                      );
                  ...
                  <div className="hidden lg:block bg-white rounded-b-[2rem] border border-t-0 border-slate-100 overflow-hidden mb-6">
                          ...
                          return (
                            <tr key={order._id} className="group hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => onOrderClick && onOrderClick(order._id)}>
                              <td className="px-8 py-5">
                                <span className="text-sm font-black text-slate-900 tracking-tighter">#{order.orderNumber ? formatOrderNumber(order.orderNumber) : '----'}</span>
                              </td>
                              <td className="px-6 py-5">
                                <div className="flex flex-col">
                                  <span className="text-[12px] font-bold text-slate-500 uppercase">{fmtDate(order.createdAt)}</span>
                                  <span className="text-[11px] text-slate-400 font-bold">{fmtTime(order.createdAt)}</span>
                                </div>
                              </td>
                              <td className="px-6 py-5">
                                <span className={cn("text-sm font-bold uppercase", order.customerName ? "text-slate-900" : "text-slate-300")}>
                                  {order.customerName || 'Walk-in'}
                                </span>
                              </td>
                              <td className="px-6 py-5">
      ...
      {/* --- Pagination Controls --- */}
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white border border-slate-200 p-4 lg:p-6 rounded-2xl lg:rounded-[2rem] shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="px-4 lg:px-6 py-2 bg-slate-50 text-slate-900 rounded-lg lg:rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-200 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-slate-100 transition-all"
            >
              Prev
            </button>
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl text-[10px] font-black transition-all",
